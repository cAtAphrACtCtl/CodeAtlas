  ...
  async searchRepository(repo: string, request: SymbolSearchRequest): Promise<SymbolRecord[]> {
    ...
    const filtered = symbols
      .filter((symbol) => {
        ...
        if (exactMode) {
          const matched = (symbol.name ?? "").toLowerCase() === request.query.toLowerCase();
          if (!matched) {
      ...
      .sort((left, right) => scoreSymbol(right, request.query, exactMode) - scoreSymbol(left, request.query, exactMode));
