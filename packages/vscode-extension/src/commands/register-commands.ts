...
} from "../providers/repository-picker.js";
...
export function registerCodeAtlasCommands(
	context: vscode.ExtensionContext,
): void {
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"codeatlas.registerCurrentWorkspace",
			async () => {
				const workspaceRoot = await getWorkspaceRoot();
				...
				vscode.window.showInformationMessage(
					`CodeAtlas registered workspace '${folderName}'.`,
				);
			},
		),

		vscode.commands.registerCommand(
			"codeatlas.discoverRepositories",
			async () => {
				const workspaceRoot = await getWorkspaceRoot();
				const services = await getServices();
				const parentFolder = path.dirname(workspaceRoot);
				const candidates =
					await services.discoveryService.discoverRepositories(parentFolder);
				...
				vscode.window.showInformationMessage(
					`CodeAtlas registered '${selected.label}'.`,
				);
			},
		),

		vscode.commands.registerCommand(
			"codeatlas.showRepositoryStatus",
			async () => {
				const services = await getServices();
				...
				await vscode.window.showQuickPick(
					toRepositoryStatusQuickPickItems(repositories, statuses),
					{
