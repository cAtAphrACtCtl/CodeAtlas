...
const boundaryQueries = ["root-hit", "hidden-hit", "node-hit", "dist-hit", "data-hit", "next-hit", "big-hit", "bin-hit"];
...
async function createBoundaryFixture(t: test.TestContext) {
  const repositoryRoot = await mkdtemp(path.join(os.tmpdir(), "codeatlas-rg-boundary-"));
  t.after(async () => {
  ...
  await writeFile(path.join(repositoryRoot, ".hidden.txt"), "hidden-hit\n", "utf8");
  await writeFile(path.join(repositoryRoot, "node_modules", "pkg", "index.js"), "node-hit\n", "utf8");
  await writeFile(path.join(repositoryRoot, "dist", "output.txt"), "dist-hit\n", "utf8");
  await writeFile(path.join(repositoryRoot, "data", "cache.txt"), "data-hit\n", "utf8");
  await writeFile(path.join(repositoryRoot, ".next", "server.txt"), "next-hit\n", "utf8");
  await writeFile(path.join(repositoryRoot, "big.txt"), `${"x".repeat(270_000)}\nbig-hit\n`, "utf8");
  await writeFile(path.join(repositoryRoot, "binary.bin"), Buffer.from([0x62, 0x69, 0x6e, 0x2d, 0x68, 0x69, 0x74, 0x00]));
...
async function collectSearches(
  ...
  for (const query of boundaryQueries) {
    const hits = await backend.searchRepository(repository, { query, limit: 10 });
