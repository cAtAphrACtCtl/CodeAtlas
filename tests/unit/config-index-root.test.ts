...
import { defaultConfig, loadConfig } from "../../packages/core/src/configuration/config.js";
...
test("loadConfig derives Zoekt indexRoot from top-level indexRoot when lexicalBackend.indexRoot is not set", async (t) => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "codeatlas-config-derive-"));
  t.after(async () => {
...
test("loadConfig uses explicit lexicalBackend.indexRoot when provided", async (t) => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "codeatlas-config-explicit-"));
  t.after(async () => {
    await rm(tempDir, { recursive: true, force: true });
  });
...
test("loadConfig falls back to built-in default when neither indexRoot is set", async (t) => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "codeatlas-config-default-"));
  t.after(async () => {
    await rm(tempDir, { recursive: true, force: true });
  });
