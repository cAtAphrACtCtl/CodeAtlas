...
export class BootstrapRipgrepLexicalSearchBackend
	implements LexicalSearchBackend
{
	...
	async prepareRepository(
		...
		const status: RepositoryIndexStatus = {
			...
			detail: rgAvailable
				? "Lexical search available via ripgrep"
		...
		if (rgAvailable) {
			try {
				...
				if (!this.backendConfig.fallbackToNaiveScan) {
					throw new Error(
						`ripgrep search failed for repository ${repository.name}`,
					);
				}
			}
		}
	...
	private async searchWithNaiveScan(
		repository: RepositoryRecord,
		request: BackendSearchRequest,
	): Promise<BackendSearchHit[]> {
